...
import tqdm
import json
from GAF.common import videotransforms
from GAF.common.anet_dataset import get_video_info, load_json
from GAF.anet.BDNet import BDNet
from GAF.common.segment_utils import softnms_v2
...
rgb_checkpoint_path = 'models/anet/500/checkpoint-12.ckpt'
flow_checkpoint_path = 'models/anet/flow/checkpoint-6.ckpt'
...
cuhk_data = load_json('./cuhk_val_simp_share.json')
...
def sub_processor(lock, pid, video_list):
    ...
    flow_net.load_state_dict(torch.load(flow_checkpoint_path),strict = False)
    rgb_net.eval().cuda()
    flow_net.eval().cuda()
    ...
    for video_name in video_list:
        ...
        for offset in offsetlist:
            ...
            with torch.no_grad():
                video_feature = rgb_net(rgb_clip, mode = 'bone')
                feat_dict = video_feature
                attention_5c = rgb_net(video_feature, mode = 'att')
                feature_5c = video_feature['Mixed_5c']
                
                attention_5c = attention_5c.unsqueeze(-1).unsqueeze(-1)
              
                feature_5c = (feature_5c * attention_5c)/(attention_5c.sum())
