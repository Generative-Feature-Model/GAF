...
def resume_training(resume, model, optimizer):
    ...
    if resume > 0:
        ...
        train_path = os.path.join(train_state_path, 'checkpoint_{}_new.ckpt'.format(resume))
...
def run_one_epoch(epoch, net, optimizer, cvae, optimizer_cvae, data_loader, epoch_step_num, training=True):
    ...
    with tqdm.tqdm(data_loader, total=epoch_step_num, ncols=0) as pbar:
        for n_iter, (clips, targets, scores, ssl_clips, ssl_targets, flags) in enumerate(pbar):
            ...
            means_4f, log_var_4f, z_4f, recon_feature_4f = cvae('forward', video_feature['Mixed_4f'], attention_4f)
...
##########################     loss , back    ##############################
    ...
    with tqdm.tqdm(data_loader, total=epoch_step_num, ncols=0) as pbar:
        for n_iter, (clips, targets, scores, ssl_clips, ssl_targets, flags) in enumerate(pbar):
            ...
            feature_bg_4f = (feature_4f * (1 - attention_4f)) / ((1 - attention_4f).sum())
            ...
            if flags[0]:
                ...
                ssl_attention_5c, ssl_attention_4f = net(ssl_video_feature, mode = 'att')
                ...
                ssl_feature_fg_4f = (ssl_feature_4f * ssl_attention_4f) / (ssl_attention_4f.sum())
                ...
                loss_trip = forward_one_epoch(net, feat_dict, ssl_targets, training=training,
                                              ssl=True)
